...
class RTLogger:
    ...
    def _get_node_info(self):
        if self.rc.get("distributed"):
            return f"rank[{self.rc.get('rank')}/" f"{self.rc.get('world_size')-1}]"
        else:
            return ""

    def _get_run_state(self):
        return f"{self.state.get('run_state'):^7}"

    def _get_task_info(self):
        return f"[{self.state.get('task_num')}/" f"{self.state.get('num_tasks')}]"

    def _get_epoch_info(self):
        return f"[{self.state.get('epoch_num')}/" f"{self.state.get('num_epochs')-1}]"

    def _get_batch_info(self):
        return f"[{self.state.get('batch_num')}/" f"{self.state.get('num_batches')}]"
    ...
    def __getattr__(self, __name: str):
        if __name.startswith("node_"):
            ...
            raise AttributeError(
                f"'{__class__.__name__}' object has no attribute '{__name}'"
            )
