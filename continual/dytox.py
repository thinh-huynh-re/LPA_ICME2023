...
class DyTox(nn.Module):
    ...
    def update_model(self, nb_new_classes):
        ...
        trunc_normal_(new_task_token, std=0.02)
    ...
    def _get_ind_clf_dim(self):
        ...
        if self.individual_classifier == "1-1":
            ...
            raise NotImplementedError(
                f"Unknown ind classifier {self.individual_classifier}"
            )
    ...
    def freeze(self, names):
        ...
        for name in names:
            if name == "all":
                ...
                cutils.set_parameters(
                    self.task_tokens[:-1], requires_grad=requires_grad
                )
    ...
    def param_groups(self):
        return {
            ...
            if self.individual_classifier
            else self.head.parameters(),
            "new_head": self.head[-1].parameters()
            if self.individual_classifier
            else self.head.parameters(),
            "head": self.head.parameters(),
            "head_div": self.head_div.parameters()
    ...
    def epoch_log(self):
        ...
        mean_dist, min_dist, max_dist = [], float("inf"), 0.0
        with torch.no_grad():
            for i in range(len(self.task_tokens)):
                for j in range(i + 1, len(self.task_tokens)):
                    dist = torch.norm(
                        self.task_tokens[i] - self.task_tokens[j], p=2
                    ).item()
    ...
    def forward_tabs_jointtokens(self, x):
        ...
        task_tokens = torch.cat(
            [task_token.expand(B, 1, -1) for task_token in self.task_tokens], dim=1
        )
    ...
    def train_forward(
        ...
        **kwargs,
        ...
        if teacher_model is not None:
            distill_loss = losses.distillation_loss(
                logits,
                old_logits,
                tasks,
                task_id,
        ...
        if div_logits is not None:
            div_loss = losses.divergence_loss(
                logits, div_logits, targets, criterion, args.head_div, kwargs["lam"]
            )
