...
def distillation_loss(
    logits, old_logits, tasks, task_id, cls_loss, auto_kd: bool, tau, kd_factor=1.0
):
    mask = tasks < task_id
    if not mask.any():
...
def relation_distill_loss(
    ...
    block_sim = _calc_similarity_graph(
        query_feats, key_feats, query_mask=mask, key_mask=key_mask
    )
    old_block_sim = _calc_similarity_graph(
        old_query_feats, old_key_feats, query_mask=mask, key_mask=key_mask
    )

    lbd = old_logits.shape[1] / logits.shape[1]
    for i in range(0, len(block_sim)):
        distill_loss += (
            F.kl_div(block_sim[i].log(), old_block_sim[i].detach(), reduction="mean")
            * lbd
        )
...
def back_distill_loss(
    query_feats, key_feats, tasks, task_id, logits, old_logits, factor
):
    ...
    if old_logits is not None:
        ...
        for i in range(1, len(block_sim)):
            distill_loss_array = F.kl_div(
                block_sim[i].log(), block_sim[i - 1].detach(), reduction="none"
            ).sum(-1)
            distill_loss_array[tasks < task_id] *= lbd
        ...
        for i in range(1, len(block_sim)):
            distill_loss += F.kl_div(
                block_sim[i].log(), block_sim[i - 1].detach(), reduction="batchmean"
            )

    return distill_loss * factor


def back_distill_loss_v2(
    query_feats, key_feats, tasks, task_id, logits, old_logits, factor
):
    ...
    if old_logits is not None:
        ...
        for i in range(1, len(block_sim)):
            distill_loss_array = F.kl_div(
                block_sim[i].log(), base_sim, reduction="none"
            ).sum(-1)
            distill_loss_array[tasks < task_id] *= lbd
            distill_loss_array[tasks == task_id] *= 1 - lbd
            distill_loss += distill_loss_array.mean(0)
        ...
        for i in range(1, len(block_sim)):
            distill_loss += F.kl_div(
                block_sim[i].log(), base_sim, reduction="batchmean"
            )
...
def forward_distill_loss(
    ...
    if old_logits is not None:
        ...
        for i in range(1, len(block_sim)):
            distill_loss_array = F.kl_div(
                block_sim[i - 1].log(), block_sim[i].detach(), reduction="none"
            ).sum(-1)
            distill_loss_array[tasks < task_id] *= lbd
            distill_loss_array[tasks == task_id] *= 1 - lbd
            distill_loss += distill_loss_array.mean(0)
        ...
        for i in range(1, len(block_sim)):
            distill_loss += F.kl_div(
                block_sim[i - 1].log(), block_sim[i].detach(), reduction="batchmean"
            )
...
def _calc_similarity_graph(query_feats, key_feats, query_mask=None, key_mask=None):
    ...
    for feat_q, feat_k in zip(query_feats, key_feats):
        ...
        # gfeat_q = feat_q.mean(1) # global query feat, avg across patches
        gfeat_q = feat_q
        gfeat_q = gfeat_q - gfeat_q.mean(
            (0, 1)
        )  # zero centered global query feat, avg across the batch
        # gfeat_k = feat_k.mean(1)
        gfeat_k = feat_k
        ...
        # sim = (gfeat_q @ gfeat_k.T / Bq).softmax(-1)
        sim = (
            (gfeat_q.permute(1, 0, 2) @ gfeat_k.permute(1, 2, 0) / Bq)
            .softmax(-1)
            .mean(0)
        )
        block_sim.append(sim)
