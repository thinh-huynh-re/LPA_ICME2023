...
def bce_with_logits(x, y):
    return F.binary_cross_entropy_with_logits(x, torch.eye(x.shape[1])[y].to(y.device))


def bce_smooth_pos_with_logits(smooth):
    def _func(x, y):
        return F.binary_cross_entropy_with_logits(
            x, torch.clamp(torch.eye(x.shape[1])[y].to(y.device) - smooth, min=0.0)
        )
...
def bce_smooth_posneg_with_logits(smooth):
    def _func(x, y):
        return F.binary_cross_entropy_with_logits(
            x,
            torch.clamp(torch.eye(x.shape[1])[y].to(y.device) + smooth, max=1 - smooth),
        )
