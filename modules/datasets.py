...
def build_dataset(is_train, args, noaug=False):
    transform = build_transform(is_train, args, noaug)

    if args.data_set.lower() == "cifar100":
        ...
        dataset = ImageNet100(
            ...
            data_subset=os.path.join(
                args.data_path,
                "imagenet100_splits",
                "train_100.txt" if is_train else "val_100.txt",
            ),
        )
...
def build_transform(is_train, args, noaug=False):
    if args.aa == "none":
        args.aa = None
