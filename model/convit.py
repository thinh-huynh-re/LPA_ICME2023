...
class ClassAttention(nn.Module):
    ...
    def forward(self, x, num_cls=1, attn_mask=None, **kwargs):
        B, N, C = x.shape
        q = (
            self.q(x[:, :num_cls])
        ...
        if attn_mask is not None:
            # attn_mask = attn_mask.expand(B, self.num_heads, -1, N)
            # attn = attn * attn_mask
            attn = attn + attn_mask
...
class TaskAttentionBlock(nn.Module):
    ...
    def forward(self, x, num_ct=1, attn_mask=None):
        cls_token = x[:, :num_ct]
        xx = self.norm1(x)
        xx = self.attn(xx, num_cls=num_ct, attn_mask=attn_mask)

        # !!! drop path should be separate for each token to be consistent with no joint tokens.
        def sep_droppath(x):
            cls_tokens = []
            for idx in range(num_ct):
                cls_tokens.append(self.drop_path(x[:, idx : idx + 1]))
            return torch.cat(cls_tokens, dim=1)

        cls_token = cls_token + sep_droppath(xx[:, :num_ct])
        cls_token = cls_token + sep_droppath(self.mlp(self.norm2(cls_token)))
